...
%--------------------------------------------------------------------------
    ...
    methods(Access = protected, Sealed)
        function nofinish = NotTerminated(obj,Population,Problem)%%  Problem
        ...
        %       end
            ...
            if ~nofinish
                ...
                %
                non1    = find(PopObj(:,1)~=0);
                PopObj  = PopObj(non1,:);
                %
                PopDec  = Population.best.decs;
                PopDec  = logical(PopDec(non1,:));
                PopObjTest = zeros(size(PopDec,1),2);
                %
                for i = 1 : size(PopObj,1)
                    [~,Rank] = sort(pdist2(Problem.TestIn(:,PopDec(i,:)),Problem.TestIn(:,PopDec(i,:))),2);
                    [~,Out]  = max(hist(Problem.TestOut(Rank(:,1:3))',Problem.Category),[],1);
                    Out      = Problem.Category(Out);
                    PopObjTest(i,1) = mean(PopDec(i,:));
                    PopObjTest(i,2) = mean(Out~=Problem.TestOut);
                end
                % 计算训练集得HV值
%                 train_HV = HVV(PopObj,[1,1]);
                mean_test_error = mean(PopObjTest(:,2));
                % 计算测试集的HV结果
                Best = NDSort(PopObjTest,1) == 1;
                PopObjTest = PopObjTest(Best,:);
                test_HV = HVV(PopObjTest,[1,1]);
                %
                formatSpec = '=================================\n mean_size: %f ## Train_error: %f ## Test_error: %f ## runtime: %f ## HV_test: %f\n';
                fprintf(formatSpec,mean(PopObj(:,1)),mean(PopObj(:,2)),mean_test_error,obj.metric.runtime,test_HV);
            end
